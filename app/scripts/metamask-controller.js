...
module.exports = class MetamaskController extends EventEmitter {
  ...
  getApi() {
    ...
    return {
      ...
      setRestoredFromThreeBoxToFalse: nodeify(
        threeBoxController.setRestoredFromThreeBoxToFalse,
        threeBoxController
      ),
  ...
  async submitPassword(password) {
    ...
    const threeBoxFeatureFlagTurnedOn = this.preferencesController.getFeatureFlags()
      .threeBox;
  ...
  async connectHardware(deviceName, page, hdPath) {
    ...
    const accountsToTrack = [
      ...new Set(oldAccounts.concat(accounts.map(a => a.address.toLowerCase())))
    ];
      ...
      if (!HW_WALLETS_KEYRINGS.includes(keyring.type)) {
        ...
        switch (version) {
          ...
          case "V3":
            ...
            break;
          case "V4":
            signature = sigUtil.signTypedData_v4(privKey, {
              data: JSON.parse(cleanMsgParams.data)
            });
            break;
        }
  ...
  setupUntrustedCommunication(connectionStream, originDomain) {
    ...
    // connect features
    const publicApi = this.setupPublicApi(
      mux.createStream("publicApi"),
      originDomain
    );
  ...
  setupPublicConfig(outStream, originDomain) {
    const configStore = this.createPublicConfigStore({
      ...
      checkIsEnabled: () =>
        this.providerApprovalController.shouldExposeAccounts(originDomain)
    });
