...
function mapStateToPropsFactory() {
  ...
  return function mapStateToProps(state) {
    // We should **not** change the identity displayed here even if it changes
    // from underneath us. If we do, we will be showing the user one private key
    // and a **different** address and name. Note that the selected identity
    // **will** change from underneath us when we unlock the keyring which is
    // the expected behavior that we are side-stepping.
    selectedIdentity = selectedIdentity || getSelectedIdentity(state)
    return {
      warning: state.appState.warning,
          privateKey: state.appState.accountDetail.privateKey,
          network: state.metamask.network, selectedIdentity,
...
ExportPrivateKeyModal.contextTypes = {
  t : PropTypes.func,
                                     ...
                                     module.exports =
    connect(mapStateToPropsFactory, mapDispatchToProps)(ExportPrivateKeyModal)

ExportPrivateKeyModal.prototype.exportAccountAndGetPrivateKey =
  ...
  exportAccount(password, address)
    ...
    ExportPrivateKeyModal.prototype.renderPasswordLabel =
        function(privateKey) {
  return h('span.private-key-password-label',
           privateKey ? this.context.t('copyPrivateKey')
                      : this.context.t('typePassword'))
}

        ExportPrivateKeyModal.prototype.renderPasswordInput =
            function(privateKey) {
  const plainKey = privateKey && stripHexPrefix(privateKey)

  return privateKey ? h(ReadOnlyInput, {
    ...
    onClick : () => copyToClipboard(plainKey),
            ...
            ExportPrivateKeyModal.prototype.renderButtons =
                function(privateKey, address, hideModal) {
  ...
  return h(AccountModalContainer, {
    ...
    backButtonAction : () => showAccountDetailModal(),
           ...
           [
             ...
             h('div.private-key-password-warning',
